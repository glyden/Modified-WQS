...
# with competing methods (WQS, linear regression) in a setting with 10 correlated predictors, five of which are truly associated with 
# the outcome, all equally.
...
path=paste0("~/Desktop/SWSR/Sim-5True/Cond",c,"/Scripts/")
...
beta=3
betas = c(rep(beta,5),rep(0,p-5)) # same coefficient for first five predictors, not including intercept
beta0 = 10
betas = c(beta0,betas)
...
#### SIMULATION CODE ####
set.seed(as.numeric(paste0(2,c,job)))
...
# compute true index
index = (beta/sum(betas[-1]))*rowSums(x[,1:5])
...
if (length(swsr.cp.fit)!=1) {
  ...
  swsr.cp.index = x %*% swsr.cp.weights # estimate index for all x
...
qindex = (beta/sum(betas[-1]))*rowSums(x.q[,1:5])
