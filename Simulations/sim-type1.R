...
load("~/Desktop/Github/Simulations/cor.matrix.Rdata")

library(mvtnorm)
library(gWQS)

# read in args, a two-item vector with condition c (correlation) and job number for simulation (e.g. 1 to 1000)
...
# set path
path="~/Desktop/Sim-Type1/Results/"

# create conditions
conditions = data.frame(condition=1:4,true.pairwise.cov=c(.2,.5,.8,NA))
true.pairwise.cov = conditions$true.pairwise.cov[c]

# set parameters for data generation
n=300; p=9; my.mu = rep(10,p)

# fix coefficients of true model (slopes of zero)
beta0 = 10
...
# generate data
my.sigma = if (c==4) {cor.matrix} else {make.sigma(true.pairwise.cov,p)}
...
# get pr.t1 results - t for "training and test"; 1 for L1
...
if (length(pr.p1.fit)!=1) {
  ...
  perm.results = tryCatch(perm.test(pr.p1.sumbeta,x,y,lambda="lambda.min",nperms=N),
                                   error=function(e) {return(NA)})
  pr.p1.permN = perm.results$N
  pr.p1.sumbeta.p = perm.results$pvalue
  ...
  # perm.results.1se = tryCatch(perm.test(pr.p1.sumbeta.1se,x,y,lambda="lambda.1se",nperms=N),
  #                                  error=function(e) {return(NA)})
  # pr.p1.permN.1se = perm.results.1se$N
  # pr.p1.sumbeta.p.1se = perm.results.1se$pvalue
...
if (length(pr.p2.fit)!=1) {
  ...
  perm.results2 = tryCatch(perm.test(pr.p2.sumbeta,x,y,lambda="lambda.min",nperms=N,alpha=0),
                                   error=function(e) {return(NA)})
  pr.p2.permN = perm.results2$N
  pr.p2.sumbeta.p = perm.results2$pvalue
  ...
  # perm.results2.1se = tryCatch(perm.test(pr.p2.sumbeta.1se,x,y,alpha=0,lambda="lambda.1se",nperms=N),
  #                                  error=function(e) {return(NA)})
  # pr.p2.permN.1se = perm.results2.1se$N
  # pr.p2.sumbeta.p.1se = perm.results2.1se$pvalue
...
# return results
output = tryCatch(data.frame(iter=job,cond=c,true.pairwise.cov=true.pairwise.cov,
                             t(pr.t1.betas),t(pr.t1.weights),t(pr.t1.select),pr.t1.size,pr.t1.sumbeta,pr.t1.sumbeta.SE,pr.t1.sumbeta.p,#pr.t1.index.mse,
                             pr.p1.permN,t(pr.p1.betas),t(pr.p1.weights),t(pr.p1.select),pr.p1.size,pr.p1.sumbeta,pr.p1.sumbeta.p,#pr.p1.index.mse,
                             ...
                             t(wqs.t.weights),wqs.t.sumbeta,wqs.t.sumbeta.SE,wqs.t.sumbeta.p,#wqs.t.index.mse,
                             t(wqs.n.weights),wqs.n.sumbeta,wqs.n.sumbeta.SE,wqs.n.sumbeta.p,#wqs.n.index.mse,
                             t(pr.t2.betas),t(pr.t2.weights),t(pr.t2.select),pr.t2.size,pr.t2.sumbeta,pr.t2.sumbeta.SE,pr.t2.sumbeta.p,#pr.t2.index.mse,
                             pr.p2.permN,t(pr.p2.betas),t(pr.p2.weights),t(pr.p2.select),pr.p2.size,pr.p2.sumbeta,pr.p2.sumbeta.p,#pr.p2.index.mse,
