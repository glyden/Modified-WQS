### PHTHALATE CORRELATION SIMULATION ###

## This is a simulation that compares the performance of penalized weighted sum regression 
# with competing methods in a setting with nine true predictors out of nine, with equal effect sizes.
...
# read in args, a two-item vector with condition c (R^2) and job number for simulation (e.g. 1 to 1000)
...
# set path
path="~/Desktop/Sim-Phthalate/Results/"

# set conditions
conditions = data.frame(condition=1:2,true.r2=c(.1,.6))
...
beta0 = 10; beta=3
betas = rep(beta,p) # not including intercept
betas = c(beta0,betas)
...
# return results
output = tryCatch(data.frame(iter=job,true.r2=true.r2,
