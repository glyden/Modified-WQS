...
## This is a simulation for the SWSR paper that compares the Type 1 error rate of SWSR 
# (with optional quantiles and optional data splitting) with competing methods 
...
## also includes c (for "condition"), from 1 to 3, which determines rho
args = (commandArgs(TRUE))
print(args)
for (i in 1:length(args)) { eval(parse(text=args[[i]])) }

path=paste0("~/Desktop/SWSR/Sim-Type1/Cond",c,"/Scripts/")

conditions = data.frame(condition=1:3,true.pairwise.cov=c(.2,.5,.8))
...
output = tryCatch(data.frame(iter=job,
                             t(swsr.ct.betas),t(swsr.ct.weights),t(swsr.ct.select),swsr.ct.size,swsr.ct.sumbeta,swsr.ct.sumbeta.SE,swsr.ct.sumbeta.p,
                             t(swsr.cp.betas),t(swsr.cp.weights),t(swsr.cp.select),swsr.cp.size,swsr.cp.sumbeta,swsr.cp.sumbeta.p,
                             t(wqs.t.weights),wqs.t.sumbeta,wqs.t.sumbeta.SE,wqs.t.sumbeta.p,
                             t(wqs.n.weights),wqs.n.sumbeta,wqs.n.sumbeta.SE,wqs.n.sumbeta.p,
                             t(swsr.qt.betas),t(swsr.qt.weights),t(swsr.qt.select),swsr.qt.size,swsr.qt.sumbeta,swsr.qt.sumbeta.SE,swsr.qt.sumbeta.p,
                             t(swsr.qp.betas),t(swsr.qp.weights),t(swsr.qp.select),swsr.qp.size,swsr.qp.sumbeta,swsr.qp.sumbeta.p,
                             t(one.betas),t(one.p),t(all.betas),t(all.p),all.mix.p,
                             true.pairwise.cov), error=function(e) {return(NULL)})
